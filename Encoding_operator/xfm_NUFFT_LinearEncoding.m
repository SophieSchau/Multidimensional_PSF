classdef xfm_NUFFT_LinearEncoding < xfm
%   NUFFT Linear Operator for non-uniform sampling with additional linear
%   encoding
%   Forward transforms image components to encoded images to multi-coil, arbitrary non-Cartesian k-space
%   along with the adjoint (multi-coil k-space to coil combined images decoded for components)
%
%   Based on work by:
%   Mark Chiew
...
%       EMat   =    Encoding matrix
%                   Defaults to bo encoding, EMat = 1
%
%   Usage:
...
properties (SetAccess = protected, GetAccess = public)
    ...
    Nenc    =   []; % number of encodings
    EMat   =   [];
    
end

methods
function res = xfm_NUFFT_LinearEncoding(dims, coils, fieldmap_struct, k, H, varargin)
    ...
    if isempty(H)
        ...
        res.EMat   =   LinearEncodingMatrix(H, dims);
        res.Ncomp  =   size(H,2);
        res.Nenc  =   size(H,1);
    ...
    res.msize   =   [prod(res.Nd), res.Nt, res.Ncomp];
...
function b = mtimes_Toeplitz(a,T,b,w)
    ...
    for t = 1:Nt
        for enc = 1:Nenc
            ...
            tmp    =   ifftn(bsxfun(@times, T(:,:,:,t,enc),fftn(tmp))); 
...
%         res =   zeros([a.Nd, nt a.Ncomp a.Nc]);
        tmp =   zeros([a.Nd, 1 a.Nenc a.Nc]);
        tmp2 =   zeros([a.Nd, nt a.Nenc 1]);
        for t = 1:nt
            for enc = 1:Nenc
                b(:,t,enc,:) = bsxfun(@times, b(:,t, enc,:), w(:,t,enc));
                tmp(:,:,:,1,enc,:)  =   nufft_adj(squeeze(b(:,t, enc, :)), a.st(t,enc));
                tmp2(:,:,:,t,enc) = a.norm*(a.S'*tmp(:,:,:,1,enc,:));
            end
        end
        res =   reshape(tmp2, [a.Nd,nt,Nenc]); 
        clear tmp
        res = a.EMat'*res;
    ...
    %   Forward NUFFT and coil transform
        res =   zeros([a.dsize(1) nt Nenc, a.Nc ]);
        tmp = a.EMat*b;
        tmp =   a.norm*(a.S*tmp);
        ...
        for t = 1:nt
            for enc = 1:a.Nenc
                for c = 1:a.Nc
                    res(:,t,enc,c)  =   nufft(squeeze(tmp(:,:,:,t,enc,c)), st(t,enc));
                    res(:,t,enc,c)  = bsxfun(@times, res(:,t,enc,c), w(:,t,enc));
                end
            end
        end
    end
...
function res = times(a,b)
    if a.adjoint
        res =   reshape(mtimes(a,b), [a.Nd(1:2) a.Nd(3), a.Nt, a.Ncomp]);
    else
        res =   mtimes(a,b);
    end
end
